;(function ( $, window, document, undefined ) {
  ...
  //---- Inicializa smart banner para navegadores direfentes a safari en iOS
  $.fn.smartBanneriOS = function(){
    ...
    if (appBannerID != null) { 
      ...
      iPad = true; iPhone = false; safari = false;
      if ((iPad || iPhone) && (!safari)) {
        $.getJSON('https://itunes.apple.com/lookup?id=' + appBannerID + '&callback=?&country=ar', function(json) {
          if (json != null) {
  ...
  // -- Deep linking en iOs
  $.fn.openiOS = function(uuid){
    var now = new Date().valueOf();
    
    setTimeout(function () {
      if (new Date().valueOf() - now > 100) return;
      window.location = 'https://itunes.apple.com/ar/app/qubittv/id926302652?uo=4';
    }, 25);
    window.location = 'qubittv://m.qubit.tv/content/onlyapp/'+uuid+'#Intent;package=tv.qubit.android.player;scheme=qubittv;end;';
  }

  // -- Deep linking en Android
  $.fn.openAndroid = function(uuid){
    window.location = 'intent://m.qubit.tv/content/onlyapp/' + uuid + '#Intent;package=tv.qubit.android.player;scheme=qubittv;S.browser_fallback_url=end;'
  }
